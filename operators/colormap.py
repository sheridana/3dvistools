...
class ColorMap(Operator):
    ...
    color_map = BoolProperty(
            name = "Color map",
            default = False,
            description = "Add color map to objects")
    ...
    object_type = EnumProperty(
            ...
            default = "Mesh",
            description = "Type of object to apply color map to")

    map_type = EnumProperty(
            name = "Map type",
            items = [
                ("Vertex count", "Vertex count", "Vertex count"), 
                ("Surface area", "Surface area", "Surface area")
            ],
            default = "Vertex count",
            description = "Color map type to color objects by")

    hue_order = EnumProperty(
            name = "Order",
            items = order,
            default = "Low to high",
            description = "Order of color mapping (red to violet/ violet to red")

    emit_order = EnumProperty(
            name = "Order",
            items = order,
            default = "Low to high",
            description = "Order of emit mapping (0 to 5 / 5 to 0)")

    alpha_order = EnumProperty(
            name = "Order",
            items = order,
            default = "Low to high",
            description = "Order of alpha mapping (0 to 1 / 1 to 0)")

    specular_order = EnumProperty(
            name = "Order",
            items = order,
            default = "Low to high",
            description = "Order of specular mapping (0 to 1 / 1 to 0)")

    fresnel_order = EnumProperty(
            name = "Order",
            items = order,
            default = "Low to high",
            description = "Order of fresnel mapping (0 to 5 / 5 to 0)")

    hue_map = BoolProperty(
            name = "Hue map",
            default = True,
            description = "Use a hue map (hsv values)")

    hue_start = FloatProperty(
            name = "Hue start",
            default = 0,
            min = 0,
            max = 1,
            description = "Start of hue range")

    hue_end = FloatProperty(
            name = "Hue end",
            default = .65,
            min = 0,
            max = 1,
            description = "End of hue range")

    emit_map = BoolProperty(
            name = "Emit map",
            default = False,
            description = "Use an emit map")

    emit_start = FloatProperty(
            name = "Emit start",
            default = .1,
            min = 0,
            max = 5,
            description = "Start of emit range")

    emit_end = FloatProperty(
            name = "Emit end",
            default = 2,
            min = 0,
            max = 5,
            description = "End of emit range")

    alpha_map = BoolProperty(
            name = "Alpha map",
            default = False,
            description = "Use an alpha map")

    alpha_start = FloatProperty(
            name = "Alpha start",
            default = .01,
            min = 0,
            max = 1,
            description = "Start of alpha range")

    alpha_end = FloatProperty(
            name = "Alpha end",
            default = 1,
            min = 0,
            max = 1,
            description = "End of alpha range")

    specular_map = BoolProperty(
            name = "Specular map",
            default = False,
            description = "Use a specular map")
    ...
    specular_start = FloatProperty(
            ...
            min = 0,
            max = 1,
            description = "Start of specular range")
    ...
    specular_end = FloatProperty(
            ...
            min = 0,
            max = 1,
            description = "End of specular range")

    fresnel_map = BoolProperty(
            name = "Fresnel map",
            default = False,
            description = "Use a fresnel map")

    fresnel_start = FloatProperty(
            name = "Fresnel start",
            default = 2,
            min = 0,
            max = 5,
            description = "Start of fresnel range")

    fresnel_end = FloatProperty(
            name = "Fresnel end",
            default = 5,
            min = 0,
            max = 5,
            description = "End of fresnel range")
    ...
    def execute(self, context):
        ...
        if self.color_map:
            ...
            def create_map(
                ...
                LTH = (bound + (v - min_val) / corrected)
                HTL = (bound - (v - min_val) / corrected)

                if hue:
                    if order:

                        m = {
                                n: colorsys.hsv_to_rgb(LTH, 1, 1)
                                for n,v in nodes.items()
                               }
                    else:

                        m = {
                                n: colorsys.hsv_to_rgb(HTL, 1, 1)
                                for n,v in nodes.items()
                               }
